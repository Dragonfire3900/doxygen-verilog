...
static QCString makeDisplayName(const ClassDef *cd,bool includeScope)
{
  ...
  QCString n;
  if (lang==SrcLangExt::VHDL || lang==SrcLangExt::VERILOG) //TODO: VERILOG needs to be a member of  the SrcLangExt space
  {
...
/*
  ...
  TODO: Verify that this documentation makes sense
...
*/
  ...
  if (getLanguage()==SrcLangExt::VHDL || getLanguage()==SrcLangExt::VERILOG)
  {
...
void ClassDefImpl::writeSummaryLinks(OutputList &ol) const
{
  ...
  if (lang!=SrcLangExt::VHDL && lang!=SrcLangExt::VERILOG)
  {
...
void ClassDefImpl::writeDeclarationLink(OutputList &ol,bool &found,const QCString &header,bool localNames) const
{
  ...
  if (visibleInParentsDeclList())
  {
    if (!found) // first class
    {
      ...
      else if (lang==SrcLangExt::VHDL ||  lang==SrcLangExt::VERILOG)
      {
        if(lang==SrcLangExt::VERILOG)
    ...
    //ol.startMemberItem(anchor(),FALSE); //TODO: Likely old syntax
    QCString ctype ;
    if(lang==SrcLangExt::VERILOG)
    ...
    for (auto &mi : *mni)
    {
      ...
      if (cd && !md->name().isEmpty() && !md->isAnonymous())
      {
        ...
        if (
            ...
            && memberWritten)
        {
          ol.writeString("<span class=\"mlabel\">"); //TODO: Make sure this makes sense
          StringVector sl;
          if (lang==SrcLangExt::VHDL || lang==SrcLangExt::VERILOG) 
          {
